...
export const AddEntryModal: React.FC<AddEntryModalProps> = ({ isOpen, onClose, workLog, selectedDate, editingEntry }) => {
  ...
  const calculatedHours = entryType === EntryType.TimeRange ? calculateDuration(startTime, endTime) : 0;
          ...
          <form onSubmit={handleSubmit} className="p-4 space-y-4">
                ...
                {(Object.values(EntryType)).map(type => (
                  <button
                    ...
                    className={`px-4 py-2 border border-gray-300 dark:border-gray-600 text-sm font-medium transition-colors ${
                      entryType === type ? 'bg-primary text-white' : 'bg-white text-gray-700 hover:bg-gray-50 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-gray-700'
            ...
            {entryType === EntryType.TimeRange && (
              <div className="grid grid-cols-2 gap-4 items-center">
                ...
                {calculatedHours > 0 && <p className="col-span-2 text-sm text-gray-600 dark:text-gray-400 text-center transition-colors">{t('common.totalHours', { hours: calculatedHours.toFixed(2) })}</p>}
              </div>
            )}
